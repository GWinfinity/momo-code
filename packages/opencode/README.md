...
- **25+ LLM Providers** — Claude, GPT-4, Gemini, OpenRouter, Zhipu (GLM), Moonshot (Kimi), Doubao, MiniMax, StepFun, Groq, Mistral, and more
- **Custom Provider** — Plug in any OpenAI-compatible API with env vars
...
| `MOMO_ZHIPU_API_KEY` | Zhipu (智谱 GLM) key |
| `MOMO_MOONSHOT_API_KEY` | Moonshot (Kimi) key |
| `MOMO_DOUBAO_API_KEY` | Doubao (字节) key |
| `MOMO_MINIMAX_API_KEY` | MiniMax key |
| `MOMO_STEPFUN_API_KEY` | StepFun (阶跃) key |
| `MOMO_CUSTOM_BASE_URL` | Custom provider URL |
| `MOMO_CUSTOM_API_KEY` | Custom provider key |
| `MOMO_CUSTOM_MODEL` | Custom provider model |

Full list: `src/auth.ts`
...
├── provider/       # 25+ LLM provider integrations
...
- **Runtime**: 36/36 tests passed (tactic + provider)
...
**Added:**
- 6 Chinese LLM providers: Zhipu (GLM), MiniMax, Moonshot (Kimi), StepFun, Doubao
- Custom OpenAI-compatible provider (MOMO_CUSTOM_* env vars)
- 36 test cases (tactic.test.ts + provider.test.ts)
