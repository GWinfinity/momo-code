...
import { BUILTIN_TIERS, resolveTierModel, listAvailableProviders, } from "../../provider/provider.js";
import { fetchModelCatalog, getModelsUrl } from "../../provider/models.js";
/**
...
export function listModels() {
    return Effect.gen(function* () {
        ...
        for (const tier of tiers) {
            const models = BUILTIN_TIERS[tier.name];
            console.log(`  ${tier.color}${tier.label}${tier.reset}: ${models.join(", ")}`);
        }
        console.log("");
        ...
        if (catalog._tag === "Right") {
            const data = catalog.right;
            if (data.models && data.models.length > 0) {
                console.log("Remote Catalog Models:");
                console.log("");
                for (const model of data.models) {
                    const statusColor = model.status === "available"
                        ? "\x1b[32m"
                        : model.status === "deprecated"
                            ? "\x1b[33m"
                            : "\x1b[31m";
                    const reset = "\x1b[0m";
                    console.log(`  ${model.id} (${model.provider}) ${statusColor}[${model.status || "unknown"}]${reset}`);
 ...
 */
export function showModelInfo(modelId) {
    return Effect.gen(function* () {
        console.log("");
        ...
        if (modelId in BUILTIN_TIERS) {
            const tier = BUILTIN_TIERS[modelId];
 ...
 */
export function runModelsCommand(args) {
    return Effect.gen(function* () {
