...
import { Evolve, CollectorLive, DistillerLive, SelectorLive, InjectorLive, SolidifyLive, GateLive, BridgeLive, ExperienceStoreLive, ExperienceGuardLive, } from "../../experience/index.js";
import { SignalScorer } from "../../evolve/signals.js";
export function showEvolveHelp() {
    console.log(`
...
function parseArgs(args) {
    const r = { mode: undefined, sessionId: `evo_${Date.now()}`, inject: false, solidify: false, list: false, help: false };
    for (const a of args) {
        if (a === "--help" || a === "-h")
            r.help = true;
...
export async function runEvolveCommand(args) {
    ...
    if (opts.inject) {
        ...
        const ctx = { id: opts.sessionId, description: "Injecting", signals: [SignalScorer.fromExitCode(0, "bash")] };
        const result = await Effect.runPromise(InjectForTask(ctx).pipe(Effect.provide(SelectorLive), Effect.provide(InjectorLive), Effect.provide(ExperienceStoreLive)));
    ...
    if (opts.solidify) {
        ...
        await Effect.runPromise(SolidifyHook(opts.sessionId, "pass", []).pipe(Effect.provide(SolidifyLive), Effect.provide(ExperienceStoreLive), Effect.provide(ExperienceGuardLive)));
    ...
    const result = await Effect.runPromise(Evolve({ sessionId: opts.sessionId, signals, mode: opts.mode || "balanced" })
