...
export const PROMPT_RULES = [
    {
        match: (modelId) => modelId.startsWith("momo-coder-"),
        promptFile: MOMO_PROMPT_FILE,
        description: "momo Code proprietary model series",
    },
    {
        match: (modelId) => modelId.includes("claude") || modelId.includes("sonnet") || modelId.includes("haiku"),
        promptFile: ANTHROPIC_PROMPT_FILE,
        description: "Anthropic Claude models",
    },
    {
        match: (modelId) => modelId.includes("gpt-"),
        promptFile: GPT_PROMPT_FILE,
        description: "OpenAI GPT models",
    },
    {
        match: (modelId) => modelId.includes("gemini"),
 ...
 */
export function resolvePromptFile(modelId) {
 ...
 */
export function getPromptSearchPaths() {
    const paths = [];
 ...
 */
export function findPromptFile(filename) {
 ...
 */
export function loadSystemPrompt(modelId) {
    return Effect.gen(function* () {
        ...
        for (const dir of searchPaths) {
            ...
            if (fs.existsSync(fullPath)) {
                const content = yield* Effect.tryPromise({
                    ...
                    catch: (error) => new Error(`Failed to read prompt file ${fullPath}: ${error instanceof Error ? error.message : String(error)}`),
                });
                return content;
            }
        }
        ...
        for (const dir of searchPaths) {
            ...
            if (fs.existsSync(fullPath)) {
                const content = yield* Effect.tryPromise({
                    try: () => fs.promises.readFile(fullPath, "utf-8"),
                    catch: (error) => new Error(`Failed to read default prompt file ${fullPath}: ${error instanceof Error ? error.message : String(error)}`),
                });
                return content;
            }
        }
 ...
 */
export function loadMomoSystemPrompt() {
    return Effect.gen(function* () {
        const searchPaths = getPromptSearchPaths();
        for (const dir of searchPaths) {
            ...
            if (fs.existsSync(fullPath)) {
                const content = yield* Effect.tryPromise({
                    try: () => fs.promises.readFile(fullPath, "utf-8"),
                    catch: (error) => new Error(`Failed to read momo prompt file: ${error instanceof Error ? error.message : String(error)}`),
                });
                return content;
            }
        }
 ...
 */
export function getMomoDefaultPrompt() {
    return [
 ...
 */
export function isClaudeCodeInheritanceEnabled() {
    if (process.env.MOMO_CLAUDE_CODE_INHERIT === "false")
        return false;
    if (process.env.MOMO_NO_CLAUDE_PROMPTS === "true")
        return false;
    return true;
}
/**
