 ...
 */
export class Miner extends Effect.Service()("evolve/Miner", {
    effect: Effect.gen(function* () {
         ...
         */
        const diagnose = (sessions) => Effect.gen(function* () {
            ...
            for (const c of clusters) {
                yield* Effect.log(`  ${c.name}: ${c.count} instances, avg retries: ${c.avgRetries.toFixed(1)}, fixable: ${c.fixable}`);
            }
            ...
            const taxonomy = {
                ...
                fixableRatio: clusters.filter((c) => c.fixable).length /
                    Math.max(clusters.length, 1),
            };
            yield* Effect.log(`[Miner] Taxonomy: ${clusters.length} clusters, fixable ratio: ${(taxonomy.fixableRatio * 100).toFixed(1)}%`);
            return taxonomy;
        });
        return { diagnose };
    }),
...
function clusterSignals(signals) {
    ...
    return Array.from(clusters.entries())
        ...
        name,
        count: sigs.length,
        avgRetries: sigs.reduce((sum, s) => sum + (s.type === "retry-loop" ? 3 : 1), 0) / sigs.length,
        signals: sigs,
 ...
 */
function extractClusterKey(signal) {
    ...
    if (ctx.includes("undefined") ||
        ctx.includes("null") ||
        ctx.includes("cannot read"))
        return "null-reference";
    // Async/await misuse
    if (ctx.includes("async") ||
        ctx.includes("await") ||
 ...
 */
function categorizeCluster(name) {
    ...
    if (name.includes("test"))
        return "logic";
    if (name.includes("external") || name.includes("network"))
        return "external";
    return "semantic";
}
