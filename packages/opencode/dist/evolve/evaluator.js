...
export class Evaluator extends Effect.Service()("evolve/Evaluator", {
    effect: Effect.gen(function* () {
        ...
        const run = (modelId, evalSet) => Effect.gen(function* () {
            ...
            yield* Effect.log(`  pass@1: ${(score.passAt1 * 100).toFixed(1)}%`);
            yield* Effect.log(`  regressions: ${score.regressionCount}`);
            yield* Effect.log(`  safety: ${(score.safetyScore * 100).toFixed(1)}%`);
            return score;
        });
        /**
         * Run full evaluation with per-task details.
         */
        const runDetailed = (modelId, evalSet) => Effect.gen(function* () {
            yield* Effect.log(`[Evaluator] Detailed eval of ${modelId} on ${evalSet}`);
         ...
         */
        const ratchetGate = (node, opts) => Effect.gen(function* () {
            const checkpoints = yield* Ref.get(checkpointsRef);
            yield* Effect.log(`[Ratchet] Checking gate for ${node.pi.H.baseModel} (eps=${opts.eps})`);
            if (checkpoints.length === 0) {
                yield* Effect.log("[Ratchet] No previous checkpoints — first promotion auto-passes");
                return true;
            }
            const prevModel = checkpoints[checkpoints.length - 1];
            // Check 1: Current eval set comparison
            const currentResult = yield* runDetailed(node.pi.H.baseModel, "momo-bench-v1");
            ...
            const currentOk = currentScore >= prevScore - opts.eps / 320; // eps=2 on 320 tasks
            yield* Effect.log(`[Ratchet] Current set: ${currentOk ? "PASS ✅" : "FAIL ❌"} ` +
            ...
            yield* Effect.log(`[Ratchet] Historical: ${historicalOk ? "PASS ✅" : "FAIL ❌"} ` +
            ...
            yield* Effect.log(`[Ratchet] Safety: ${safetyOk ? "PASS ✅" : "FAIL ❌"} ` +
                `(score: ${currentResult.score.safetyScore.toFixed(3)})`);
            const overallPass = currentOk && historicalOk && safetyOk;
            yield* Effect.log(`[Ratchet] Overall: ${overallPass ? "GATE OPEN ✅" : "GATE CLOSED ❌"}`);
            return overallPass;
        });
        /**
         * Record a checkpoint's evaluation results for future ratchet checks.
         */
        const recordCheckpoint = (modelId, results) => Effect.gen(function* () {
         ...
         */
        const getHistory = (modelId) => Ref.get(historyRef).pipe(Effect.map((h) => {
