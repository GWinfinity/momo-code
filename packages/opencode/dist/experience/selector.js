...
import { winRate, } from "./tactic.js";
import { matchSignalPattern } from "./signals.js";
import { ExperienceStore } from "./store.js";
/** Default selection options. */
export const defaultSelectionOpts = {
    k: 6,
    method: "thompson",
    budgetTokens: 2048,
    minWinRate: 0.1,
 ...
 */
export class Selector extends Effect.Service()("experience/Selector", {
    effect: Effect.gen(function* () {
         ...
         */
        const retrieve = (ctx, opts) => Effect.gen(function* () {
            const mergedOpts = { ...defaultSelectionOpts, ...opts };
            yield* Effect.log(`[Selector] Retrieving candidates for task with ${ctx.signals.length} signals`);
            // Fetch all active tactics from the store
            const allTactics = yield* store.loadTactics().pipe(Effect.orElse(() => Effect.succeed([])));
            yield* Effect.log(`[Selector] Store returned ${allTactics.length} active tactics`);
            const candidates = [];
            for (const tactic of allTactics) {
                ...
                const signalMatched = tactic.triggers.some((pattern) => ctx.signals.some((signal) => matchSignalPattern(signal, pattern)));
                if (signalMatched) {
                    candidates.push(tactic);
                }
            }
            yield* Effect.log(`[Selector] Found ${candidates.length} candidate tactics`);
            return candidates;
        });
        /**
         ...
         */
        const rankThompson = (candidates, ctx, opts) => Effect.sync(() => {
            const mergedOpts = { ...defaultSelectionOpts, ...opts };
            const ranked = candidates.map((tactic) => {
         ...
         * @returns Effect resolving to ranked tactics sorted by finalScore desc
         */
        const rankUcb = (candidates, ctx, opts) => Effect.sync(() => {
            const mergedOpts = { ...defaultSelectionOpts, ...opts };
            // Total uses across all candidates for the exploration term
            const totalUses = candidates.reduce((sum, t) => sum + t.stats.uses, 0);
            ...
            const ranked = candidates.map((tactic) => {
                ...
                const exploration = n === 0 ? Infinity : Math.sqrt((2 * logTotal) / n);
         ...
         */
        const select = (ctx, opts) => Effect.gen(function* () {
            const mergedOpts = { ...defaultSelectionOpts, ...opts };
            yield* Effect.log(`[Selector] Selecting tactics (method=${mergedOpts.method}, k=${mergedOpts.k}, budget=${mergedOpts.budgetTokens}t)`);
            ...
            // Step 2: Rank
            const ranked = mergedOpts.method === "thompson"
                ? yield* rankThompson(candidates, ctx, mergedOpts)
                : yield* rankUcb(candidates, ctx, mergedOpts);
            // Step 3: Apply token budget
            const selected = applyTokenBudget(ranked, mergedOpts.budgetTokens, mergedOpts.k);
            yield* Effect.log(`[Selector] Selected ${selected.length} tactics (top score=${selected[0]?.finalScore.toFixed(3) ?? "N/A"})`);
            return selected;
        });
        return {
            retrieve,
 ...
 */
function isScopeCompatible(scope, ctx) {
    if (scope === "global") {
        return true;
    }
    if (scope === "repo") {
 ...
 * - Repo match: 2.0
 */
function computeScopePriority(scope, ctx) {
    if (scope === "global") {
        return 1.0;
    }
    if (scope === "user") {
 ...
 */
function applyTokenBudget(ranked, budgetTokens, maxCount) {
    const selected = [];
