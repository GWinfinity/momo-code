...
import { ExperienceGuard } from "./guard.js";
/**
 ...
 */
export class Bridge extends Effect.Service()("experience/Bridge", {
    effect: Effect.gen(function* () {
        const store = yield* ExperienceStore;
        const guard = yield* ExperienceGuard;
        // Accumulated promoted cases waiting to be pushed
        const pendingCasesRef = yield* Ref.make([]);
        const pendingTacticsRef = yield* Ref.make([]);
         ...
         */
        const enqueueForFineTune = (tactic) => Effect.gen(function* () {
            if (tactic.status !== "promoted") {
                yield* Effect.logWarning(`[Bridge] Tactic ${tactic.id} is not promoted (status=${tactic.status}) — skipping`);
                return 0;
            }
            ...
            yield* Effect.log(`[Bridge] ${tactic.id}: ${promotedCases.length} success, ${failureCases.length} failure cases`);
            ...
            yield* Ref.update(pendingTacticsRef, (existing) => {
                // Avoid duplicates
                if (existing.some((t) => t.id === tactic.id))
                    return existing;
         ...
         */
        const buildTrainingSamples = (cases, tactics) => Effect.gen(function* () {
            yield* Effect.log(`[Bridge] Building training samples from ${cases.length} cases, ${tactics.length} tactics...`);
            ...
            yield* Effect.log(`[Bridge] Built: ${gold.length} gold, ${hardNegatives.length} hard-neg, ${replay.length} replay`);
            // Scrub before returning
            const allSamples = [...gold, ...hardNegatives, ...replay];
            const scrubbed = yield* guard.scrubBeforeStore(allSamples.map((s) => ({ ...s, context: s.context })));
            // Re-split by tag after scrubbing
            const scrubbedMap = new Map(scrubbed.map((s) => [s.id, s]));
            const scrubbedGold = gold
                .map((s) => scrubbedMap.get(s.id))
                .filter((s) => s !== undefined && s._tag === "gold");
            const scrubbedHardNeg = hardNegatives
                .map((s) => scrubbedMap.get(s.id))
                .filter((s) => s !== undefined && s._tag === "hard-negative");
            const scrubbedReplay = replay
                .map((s) => scrubbedMap.get(s.id))
                .filter((s) => s !== undefined && s._tag === "replay");
            const result = {
                gold: scrubbedGold,
         ...
         */
        const spilloverToCurriculum = (opts) => Effect.gen(function* () {
            ...
            yield* Effect.log(`[Bridge] Spillover check: ${pendingCases.length} pending cases ` +
            ...
            if (passCases.length < minCases) {
                yield* Effect.log(`[Bridge] Insufficient cases: ${passCases.length} pass cases, need ${minCases}`);
                return yield* Effect.fail(new InsufficientCasesError(passCases.length, minCases));
            }
            ...
            yield* Effect.log(`[Bridge] ✅ Spillover recorded: ${totalSamples} samples ` +
         ...
         */
        const getPendingStats = () => Effect.gen(function* () {
         ...
         */
        const forceSpillover = () => Effect.gen(function* () {
