...
import { generateTacticId } from "./tactic.js";
// ---------------------------------------------------------------------------
// Helper: content hash for dedup (SHA-256 → first 16 chars)
// ---------------------------------------------------------------------------
function contentHash(scope, title, steps) {
 ...
 */
export class Distiller extends Effect.Service()("experience/Distiller", {
    effect: Effect.gen(function* () {
         ...
         */
        const distill = (signals, opts = {}, existing = []) => Effect.sync(() => {
         ...
         */
        const distillSuccessPattern = (signals, toolName) => {
         ...
         */
        const distillFailureCluster = (signals) => {
            const dominantType = getDominantType(signals);
            return {
        ...
        return { distill, distillSuccessPattern, distillFailureCluster };
    }),
    dependencies: [],
}) {
}
...
function clusterByType(signals) {
    const clusters = {};
    for (const s of signals) {
...
function getDominantType(signals) {
    const counts = {};
    for (const s of signals) {
        counts[s.type] = (counts[s.type] || 0) + 1;
    }
    let maxType = "test-pass";
    let maxCount = 0;
...
function generateTitle(signals, toolName) {
...
function generateSteps(signals) {
    const type = getDominantType(signals);
    const stepSets = {
        "test-pass": [
...
function generateChecks(signals, toolName) {
    return [
        `npm test`,
        `npx tsc --noEmit`,
        `git diff --stat`,
...
function findExisting(draft, existing) {
    const draftHash = contentHash(draft.scope, draft.title, draft.steps);
    for (const tac of existing) {
        // Exact hash match
        const tacHash = contentHash(tac.scope, tac.title, tac.steps);
        if (tacHash === draftHash)
            return tac;
        // Semantic similarity > 0.8
        const sim = keywordSimilarity(draft.title + " " + draft.steps.join(" "), tac.title + " " + tac.steps.join(" "));
        if (sim >= 0.8)
            return tac;
    }
    return undefined;
