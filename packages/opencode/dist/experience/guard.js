...
import { Guard as EvolveGuard } from "../evolve/guard.js";
 ...
 */
export const BANNED_SHELL_PATTERNS = [
...
export class GuardViolationError {
    reason;
    command;
    _tag = "GuardViolationError";
    constructor(reason, command) {
        this.reason = reason;
 ...
 */
export class ExperienceGuard extends Effect.Service()("experience/Guard", {
    effect: Effect.gen(function* () {
         ...
         */
        const validateCheckCommand = (cmd) => Effect.gen(function* () {
            ...
            const hasAllowedPrefix = ALLOWED_CHECK_PREFIXES.some((prefix) => firstToken === prefix || firstToken.startsWith(prefix));
            if (!hasAllowedPrefix) {
                const err = new GuardViolationError(`Command prefix '${firstToken}' not in allowed list: ${ALLOWED_CHECK_PREFIXES.join(", ")}`, cmd);
                validationLog.push({ timestamp: Date.now(), command: cmd, allowed: false, reason: err.reason });
                return yield* Effect.fail(err);
            }
            ...
            for (const pattern of BANNED_SHELL_PATTERNS) {
                if (pattern.test(trimmed)) {
                    const err = new GuardViolationError(`Command contains banned shell pattern: ${pattern.source}`, cmd);
                    validationLog.push({ timestamp: Date.now(), command: cmd, allowed: false, reason: err.reason });
                    return yield* Effect.fail(err);
                }
            }
         ...
         */
        const checkStagnation = (tactic, recentCases) => Effect.gen(function* () {
            ...
            const failureSignatures = failureCases.map((c) => c.signals.map((s) => s.type).join(","));
            ...
            if (dominantCount >= stagnationThreshold) {
                const err = new StagnationError(tactic.id, `Tactic '${tactic.id}' stagnated: ${dominantCount}/${failureCases.length} failures ` +
         ...
         */
        const deduplicateSignals = (signals, windowMs = 60_000) => Effect.sync(() => {
            if (signals.length === 0)
                return [];
         ...
         */
        const scrubBeforeStore = (data) => Effect.gen(function* () {
         ...
         */
        const containsBannedPatterns = (text) => {
            for (const pattern of BANNED_SHELL_PATTERNS) {
         ...
         */
        const getValidationLog = (limit = 50) => Effect.sync(() => validationLog.slice(-limit));
        return {
