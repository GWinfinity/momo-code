 ...
 */
export function computeVerdict(signals, weights) {
    if (signals.length === 0)
        return "partial";
    let passScore = 0;
    let failScore = 0;
    for (let i = 0; i < signals.length; i++) {
        ...
        if (s.verdict === "pass")
            passScore += s.confidence * weight;
        else if (s.verdict === "fail")
            failScore += s.confidence * weight;
 ...
 */
export function signalMatchesPattern(signal, pattern) {
    // Type match
    ...
    if (pattern.matchContext) {
        ...
        if (!ctx.includes(pattern.matchContext.toLowerCase()))
            return false;
    }
    return true;
}
 ...
 */
export class Collector extends Effect.Service()("experience/Collector", {
    effect: Effect.gen(function* () {
        /**
         * Observe a session given its signals.
         * In production, this would read from MOMO_DB.
         */
        const fromSignals = (sessionId, signals) => Effect.sync(() => ({
            sessionId,
            signals,
         ...
         */
        const fromToolResult = (sessionId, toolName, exitCode) => Effect.sync(() => {
            const type = exitCode === 0
                ? "test-pass"
                : toolName === "bash"
                    ? "test-fail"
                    : "compile-error";
         ...
         */
        const aggregate = (observations) => Effect.sync(() => {
            let passCount = 0;
            let failCount = 0;
            let totalSignals = 0;
        ...
        return { fromSignals, fromToolResult, aggregate };
    }),
    dependencies: [],
}) {
}
