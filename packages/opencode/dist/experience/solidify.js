 ...
 */
export class Solidify extends Effect.Service()("experience/Solidify", {
    effect: Effect.gen(function* () {
        const store = yield* ExperienceStore;
        const guard = yield* ExperienceGuard;
        // Track rollback history
        const rollbackLogRef = yield* Ref.make([]);
        /**
         ...
         */
        const applyVerdict = (opts) => Effect.gen(function* () {
            yield* Effect.log(`[Solidify] Applying verdict='${opts.verdict}' for session=${opts.sessionId} ` +
            ...
            for (const tactic of tactics) {
                ...
                yield* Effect.log(`[Solidify] Updated ${tactic.id}: α=${updated.stats.alpha} β=${updated.stats.beta} ` +
            ...
            if (rollbackSuggested) {
                yield* Effect.logWarning(`[Solidify] Verdict=fail — suggesting rollback for session ${opts.sessionId}`);
                yield* rollbackSession(opts.sessionId).pipe(Effect.catchAll((err) => Effect.logWarning(`[Solidify] Rollback attempt failed: ${err}`)));
            }
            const result = {
                updatedTactics,
                caseRecord,
            ...
            yield* Effect.log(`[Solidify] Complete: ${updatedTactics.length} tactics updated, ` +
         ...
         */
        const rollbackSession = (sessionId) => Effect.gen(function* () {
         ...
         */
        const checkGuardrails = (tactic, changes) => Effect.gen(function* () {
            // Guardrail 1: Maximum files touched
            const MAX_FILES = tactic.guardrails.maxFiles;
            if (changes.length > MAX_FILES) {
                yield* Effect.logWarning(`[Solidify] Guardrail fail: ${tactic.id} touches ${changes.length} files (max ${MAX_FILES})`);
                return false;
            }
            ...
            for (const change of changes) {
                for (const forbidden of tactic.guardrails.forbiddenPaths) {
                    if (change.path.includes(forbidden)) {
                        yield* Effect.logWarning(`[Solidify] Guardrail fail: ${tactic.id} touches forbidden path ${forbidden} in ${change.path}`);
                        return false;
                    }
                }
            }
            ...
            for (const change of changes) {
                ...
                if (hasBanned) {
                    yield* Effect.logWarning(`[Solidify] Guardrail fail: ${tactic.id} contains banned shell patterns in ${change.path}`);
                    return false;
                }
            }
            // Guardrail 4: No secret patterns in generated content
            const hasSecrets = yield* Effect.forEach(changes, (change) => Effect.map(Effect.promise(() => Promise.resolve(/sk-[a-zA-Z0-9]{20,}/.test(change.content) ||
                /gh[pousr]_[a-zA-Z0-9]{20,}/.test(change.content) ||
                /AKIA[0-9A-Z]{16}/.test(change.content))), (hasSecret) => ({ path: change.path, hasSecret })));
            const secretMatches = hasSecrets.filter((r) => r.hasSecret);
            if (secretMatches.length > 0) {
                yield* Effect.logWarning(`[Solidify] Guardrail fail: ${tactic.id} contains potential secrets in: ` +
 ...
 */
function updateTacticStats(tactic, verdict) {
    return Effect.sync(() => {
