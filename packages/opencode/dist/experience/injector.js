...
export const defaultInjectOpts = {
 ...
 */
export class Injector extends Effect.Service()("experience/Injector", {
    effect: Effect.gen(function* () {
         ...
         */
        const formatTactic = (tactic, opts) => Effect.sync(() => {
            ...
            lines.push(`*[win rate: ${(wr * 100).toFixed(0)}%, ` +
         ...
         */
        const toPromptBlock = (tactics, opts) => Effect.gen(function* () {
            const merged = { ...defaultInjectOpts, ...opts };
            yield* Effect.log(`[Injector] Formatting ${tactics.length} tactics into prompt block`);
            if (tactics.length === 0) {
                return {
                    block: "",
                    tacticIds: [],
            ...
            if (merged.header) {
                ...
                lines.push("");
                lines.push("The following tactics were learned from past sessions. " +
            ...
            yield* Effect.log(`[Injector] Formatted ${includedIds.length} tactics (~${totalTokens} tokens)`);
         ...
         */
        const injectIntoPrompt = (block, systemPrompt) => Effect.sync(() => {
         ...
         */
        const inject = (rankedTactics, systemPrompt, injectOpts) => Effect.gen(function* () {
            ...
            yield* Effect.log(`[Injector] Injected ${result.tacticIds.length} tactics into prompt (~${result.estimatedTokens} tokens)`);
