...
export type PolicyAction =
  | { readonly _tag: "rebuild-data"; readonly reason: string }
  | { readonly _tag: "tune-hyperparams"; readonly reason: string; readonly suggestedLoraRank?: number; readonly suggestedLr?: number }
  | { readonly _tag: "surgical"; readonly reason: string; readonly targetClusters: ReadonlyArray<string> }
  | { readonly _tag: "rollback"; readonly reason: string; readonly previousScore: number; readonly currentScore: number }
  | { readonly _tag: "promote"; readonly reason: string }
 ...
 */
export class Policy extends Effect.Service<Policy>()("evolve/Policy", {
  effect: Effect.gen(function* () {
    const lastScoreRef = yield* Ref.make<Option.Option<EvalScore>>(Option.none())
    const stagnationCountRef = yield* Ref.make(0)
    const historyRef = yield* Ref.make<ReadonlyArray<EvalScore>>([])
     ...
     */
    const classify = (score: EvalScore): Effect.Effect<PolicyAction, never> => {
     ...
     */
    const isStagnated = (threshold: number): Effect.Effect<boolean, never> => {
      return Ref.get(stagnationCountRef).pipe(
     ...
     */
    const getHistory = (): Effect.Effect<ReadonlyArray<EvalScore>, never> => {
    ...
    return { classify, isStagnated, getHistory, reset } as const
  }),
  dependencies: [],
}) {}
