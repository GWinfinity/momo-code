...
export interface CurriculumOpts {
  readonly hardNegRatio: number   // Target ratio for hard negatives (0.25-0.35)
  readonly replay: boolean         // Whether to include replay slice
  readonly replayRatio?: number   // Target ratio for replay (0.10-0.20)
  readonly goldRatio?: number     // Target ratio for gold (0.40-0.60)
}
 ...
 */
export class Curriculum extends Effect.Service<Curriculum>()("evolve/Curriculum", {
  effect: Effect.gen(function* () {
     ...
     */
    const build = (spec: DataSpec, opts: CurriculumOpts) =>
      Effect.gen(function* () {
        ...
        const totalRaw =
          spec.gold.length + spec.hardNegatives.length + spec.replay.length
        ...
        yield* Effect.log(
          `[Curriculum] Target sizes: gold=${targetGold}, hard-neg=${targetHardNeg}, replay=${targetReplay}`,
        )

        // 3. Balance slices to target sizes
        const balancedGold = balanceSlice(spec.gold, targetGold)
        const balancedHardNeg = balanceSlice(
          spec.hardNegatives,
          targetHardNeg,
        )
        ...
        yield* Effect.log(
          `[Curriculum] Built: ${withCoT.length} samples ` +
     ...
     */
    const synthesizeFromClusters = (
      clusters: ReadonlyArray<ConfusionCluster>,
      sessions: ReadonlyArray<{ id: string; messages: unknown[] }>,
    ) =>
      Effect.gen(function* () {
        yield* Effect.log(
          `[Curriculum] Synthesizing hard-negatives from ${clusters.length} clusters...`,
        )

        const hardNegs: TrainingSample[] = []
        ...
        yield* Effect.log(
          `[Curriculum] Generated ${hardNegs.length} hard-negative samples`,
        )
    ...
    return { build, synthesizeFromClusters, getStats } as const
  }),
 ...
 */
function validateConstraints(spec: DataSpec): Effect.Effect<void, never> {
  return Effect.gen(function* () {
    const c = spec.qualityConstraints

    if (!c.twoForOne) {
      yield* Effect.logWarning(
        "[Curriculum] 2-for-1 rule disabled — hard negatives may not pair correctly",
      )
    }
    if (!c.labelBalance) {
      yield* Effect.logWarning(
        "[Curriculum] Label balance check disabled — class imbalance possible",
      )
    }
    if (!c.contextLengthMatch) {
      yield* Effect.logWarning(
        "[Curriculum] Context length matching disabled — train/test mismatch possible",
      )
    }
    if (!c.entityDiversity) {
      yield* Effect.logWarning(
        "[Curriculum] Entity diversity check disabled — memorization risk",
      )
    }
    if (!c.cotAnnotated) {
      yield* Effect.logWarning(
        "[Curriculum] CoT annotation disabled — reasoning quality may suffer",
      )
    }
 ...
 */
function balanceSlice<T>(samples: ReadonlyArray<T>, target: number): T[] {
...
function applyTwoForOne(
  hardNegs: TrainingSample[],
  golds: TrainingSample[],
): TrainingSample[] {
  if (golds.length === 0) return hardNegs
  ...
  for (const hn of hardNegs) {
    ...
    const paired = golds.find(
      (g) =>
        g.context.length >= hnLen * 0.8 &&
 ...
 */
function ensureLabelBalance<T extends { reason?: string }>(samples: T[]): T[] {
  if (samples.length === 0) return []
 ...
 */
function ensureEntityDiversity<T extends { context: string }>(
  samples: T[],
): T[] {
 ...
 */
function matchContextLength<T extends { context: string }>(
  samples: T[],
  spec: DataSpec,
): T[] {
 ...
 */
function annotateCoT<T extends TrainingSample>(
  samples: T[],
 ...
 */
function inferCorrectAction(cluster: ConfusionCluster): string {
  const fixes: Record<string, string> = {
    "missing-import":
      "Add the missing import statement at the top of the file. Check package.json for the correct package name.",
    "type-error":
      "Fix the TypeScript type annotation to match the expected type. Run 'tsc --noEmit' to identify the specific type mismatch.",
    "null-reference":
      "Add a null/undefined check before accessing the property. Use optional chaining (?.) or early return.",
    "async-misuse":
      "Add 'await' before the async call or properly handle the Promise with .then()/.catch().",
    "api-usage":
      "Check the API documentation for the correct signature. Update to the latest version if the API has changed.",
    "test-failure":
      "Review the test expectations and update the implementation to match. Run the specific failing test with verbose output.",
    "lint-violation":
      "Apply the linting rule fix. Run 'eslint --fix' for auto-fixable rules or manually address the reported issue.",
    "styling-error":
      "Use the correct Tailwind class or CSS property. Check the Tailwind docs for valid utility classes.",
    "react-error":
      "Check the React component for missing hooks, incorrect JSX syntax, or stale closures. Ensure all hooks follow rules of hooks.",
    "general-error":
      "Review the error message carefully. Check the stack trace for the root cause and apply the appropriate fix.",
  }
