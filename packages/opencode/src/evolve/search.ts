...
export interface SearchOpts {
  readonly explorationConstant: number   // UCT exploration constant C
  readonly decayFactor: number           // Time decay for exploration bonus
  readonly maxDepth: number              // Maximum search depth
  readonly stagnationThreshold: number   // Iterations without improvement
}
 ...
 */
export class Search extends Effect.Service<Search>()("evolve/Search", {
  effect: Effect.gen(function* () {
     ...
     */
    const init = (opts: { root: string }) =>
      Effect.gen(function* () {
        const rootId = `root-${Date.now()}`
        const now = new Date()

        // Create root pipeline from base model
        const rootPipeline: TrainingPipeline = {
          D: {
            gold: [],
            hardNegatives: [],
            replay: [],
          ...
          H: {
            ...
            epochs: 3,
            systemPrompt:
              "You are an expert coding assistant. Think step by step.",
          },
          S: {
            format: "direct",
      ...
      Effect.gen(function* () {
        ...
        while (current.children.length > 0 && current.depth < defaultOpts.maxDepth) {
          ...
          for (const childId of current.children) {
            ...
            const uctScore = computeUCT(
              child,
              current.visits,
              defaultOpts.explorationConstant,
              graph.iteration,
              defaultOpts.decayFactor,
            )
        ...
        yield* Effect.log(
          `[MCGS] Selected node ${current.id} at depth ${current.depth} (path: ${path.join(" -> ")})`,
        )
     ...
     */
    const expand = (parent: MCNode, taxonomy: Taxonomy) =>
      Effect.gen(function* () {
        const children: MCNode[] = []
        const now = new Date()
        ...
        if (taxonomy.clusters.length > 0) {
          ...
          const childPipeline: TrainingPipeline = {
            ...parent.pi,
            D: {
              ...parent.pi.D,
        ...
        if (
          taxonomy.clusters.some((c) => c.category === "logic") &&
        ...
        yield* Effect.log(
          `[MCGS] Expanded node ${parent.id} into ${children.length} children`,
        )
        for (const c of children) {
          yield* Effect.log(
            `  Child ${c.id}: ${describeChange(parent.pi, c.pi)}`,
          )
        }
    ...
    const addNode = (
      ...
      Effect.gen(function* () {
        ...
        const currentBest =
          graph.bestNodeId._tag === "Some"
            ? graph.nodes.get(graph.bestNodeId.value)
            : undefined
        const currentBestScore =
          currentBest?.score._tag === "Some" ? currentBest.score.value.passAt1 : 0
        ...
        if (opts.f.passAt1 > currentBestScore) {
          ...
          yield* Effect.log(
            `[MCGS] New best node: ${nodeId} (pass@1=${(opts.f.passAt1 * 100).toFixed(1)}%)`,
          )
        } else {
    ...
    const shouldContinue = (
      ...
      Effect.sync(() => {
        if (graph.iteration >= maxIters) return false
        if (graph.budgetUsed >= budget) return false
        return true
      })
     ...
     */
    const stagnated = (graph: MCGraph, threshold: number) =>
      Ref.get(stagnationRef).pipe(
     ...
     * local optima."
     */
    const fuseOrEvolve = (graph: MCGraph) =>
      Effect.gen(function* () {
        yield* Effect.log(
          `[MCGS] Stagnation detected — running FUSE operator...`,
        )

        // Find all leaf nodes with scores
        const leaves = Array.from(graph.nodes.values()).filter(
          (n) =>
            n.children.length === 0 &&
            n.score._tag === "Some" &&
        ...
        if (leaves.length < 2) {
          yield* Effect.log(
            `[MCGS] Not enough leaves for fusion — applying mutation instead`,
          )
          ...
          yield* Effect.log(
            `[MCGS] Mutated node ${bestNode.id} -> ${mutated.id}`,
          )
          return
        }
        ...
        leaves.sort(
          (a, b) =>
            (Option.getOrUndefined(b.score)?.passAt1 ?? 0) - (Option.getOrUndefined(a.score)?.passAt1 ?? 0),
        )

        const [leaf1, leaf2] = leaves.slice(0, 2)

        // Fuse: combine data from both, take best hyperparams
        const fusedPipeline: TrainingPipeline = {
          D: {
            ...leaf1.pi.D,
          ...
          H: {
            ...
            learningRate: Math.min(
              leaf1.pi.H.learningRate,
              leaf2.pi.H.learningRate,
            ),
          ...
          S: {
            ...
            format:
              leaf1.pi.S.format === "cot" || leaf2.pi.S.format === "cot"
                ? "cot"
                : "direct",
        ...
        yield* Effect.log(
          `[MCGS] FUSE created: ${fusedId} from ${leaf1.id} + ${leaf2.id}`,
        )
      })
     ...
     * PRUNE: Remove low-utility subtrees.
     */
    const prune = (graph: MCGraph, threshold: number) =>
      Effect.gen(function* () {
        let pruned = 0
...
function computeUCT(
  ...
  const score =
    node.score._tag === "Some" ? node.score.value.passAt1 : 0.5 // Default prior for unvisited nodes
 ...
 */
function mutate(node: MCNode): Effect.Effect<MCNode, never> {
  return Effect.sync(() => {
    const mutations = [
      // Increase LoRA rank
      (pi: TrainingPipeline): TrainingPipeline => ({
        ...pi,
 ...
 */
function estimateCost(job: TrainingJob): number {
  // Rough cost estimation based on dataset size and epochs
  // GPU hours ~= dataset_size * epochs / 1000
  // At $2/hour for A100
  const gpuHours = (job.datasetSize * (job.hparams as any).epochs) / 1000
  return gpuHours * 2.5
}
 ...
 */
function describeChange(parent: TrainingPipeline, child: TrainingPipeline): string {
