...
function parseArgs(args: string[]) {
  ...
  for (const a of args) {
    ...
    else if (["run", "status", "promote", "diagnose"].includes(a)) r.sub = a
    else if (a.startsWith("--epochs=")) r.epochs = parseInt(a.slice(9), 10)
    else if (a.startsWith("--lr=")) r.lr = parseFloat(a.slice(5))
    else if (a.startsWith("--batch=")) r.batch = parseInt(a.slice(8), 10)
    else if (a.startsWith("--lora-r=")) r.loraR = parseInt(a.slice(9), 10)
    else if (a === "--dry-run") r.dryRun = true
  }
  return r
}

export function runFinetuneCommand(args: string[]): Effect.Effect<void> {
  return Effect.gen(function* () {
    const opts = parseArgs(args)
    if (opts.help) { showFinetuneHelp(); return }
    switch (opts.sub) {
      case "diagnose": yield* doDiagnose(opts); break
      case "run": yield* doTrain(opts); break
      case "status": yield* doStatus(); break
      case "promote": yield* doPromote(opts); break
    }
  }).pipe(Effect.catchAll((e: any) => Effect.sync(() => console.error(`\x1b[31mError:\x1b[0m ${e?.message || e}`))))
}

function doDiagnose(opts: ReturnType<typeof parseArgs>): Effect.Effect<void> {
  return Effect.sync(() => {
...
function doTrain(opts: ReturnType<typeof parseArgs>): Effect.Effect<void> {
  return Effect.gen(function* () {
    console.log(`\n\x1b[1;36m🏋️ /fine-tune Training\x1b[0m`)
    if (opts.dryRun) console.log(`  \x1b[90m[DRY RUN]\x1b[0m`)
    ...
    for (const s of steps) {
      ...
      if (!opts.dryRun) yield* Effect.sleep("50 millis")
    }
    console.log(opts.dryRun ? `` : `  \x1b[32m✓\x1b[0m Done.\n`)
  }).pipe(Effect.catchAll((e: any) => Effect.sync(() => console.error(`\x1b[31mTrain error:\x1b[0m ${e?.message || e}`))))
}

function doStatus(): Effect.Effect<void> {
  return Effect.sync(() => {
...
function doPromote(opts: ReturnType<typeof parseArgs>): Effect.Effect<void> {
  return Effect.sync(() => {
