 ...
 *                                 Plan a DAG → execute subagents in parallel
...
function printUsage(): void {
  ...
  console.log(``)
  console.log(`  momo /graph run "<task>" [--max-nodes=N]  Plan a DAG → run subagents → synthesize`)
  ...
  console.log(`     MOMO_RLM_BUDGET (4), MOMO_RLM_TIMEOUT_MS (300000)`)
...
async function cmdRun(args: GraphArgs): Promise<void> {
  ...
  for (const n of run.nodes) {
    console.error(`  ${nodeMark(n.state)} ${CYAN}${n.id}${RESET} ${DIM}${n.task.slice(0, 80)}${RESET}`)
...
function cmdStatus(args: GraphArgs): void {
  ...
  console.log(`${MAGENTA}Graph "${run.id}"${RESET} — ${run.status} (${done}/${run.nodes.length} done)`)
  console.log(`  task: ${run.task.slice(0, 100)}`)
  console.log(`  created: ${run.createdAt}`)
  for (const n of run.nodes) {
    const deps = n.dependsOn.length > 0 ? ` ${DIM}after: ${n.dependsOn.join(", ")}${RESET}` : ""
    console.log(`  ${nodeMark(n.state)} ${CYAN}${n.id}${RESET} [${n.state}]${deps}`)
...
function cmdList(args: GraphArgs): void {
  ...
  for (const r of runs.slice(0, 20)) {
    ...
    console.log(
      `${CYAN}${r.id}${RESET}  ${r.status}  ${done}/${r.nodes.length} done  ${DIM}${r.createdAt}${RESET}  ${r.task.slice(0, 60)}`,
    )
  }
}
