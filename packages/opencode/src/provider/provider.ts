...
const TIER_FALLBACK_ORDER = ["ultra", "standard", "lite"] as const
...
// ---------------------------------------------------------------------------
// Model groups (upstream model selection logic)
// ---------------------------------------------------------------------------
 ...
 */
export const BUILTIN_MODELS: readonly ModelGroupEntry[] = [
  // Ultra tier
  {
    id: "claude-sonnet-4",
  ...
  {
    id: "doubao-pro-128k",
    providerModelId: "doubao-pro-128k",
    provider: "doubao",
 ...
 */
function buildModelMap(): Map<string, ModelGroupEntry> {
...
// ---------------------------------------------------------------------------
// Claude Code ecosystem inheritance
// ---------------------------------------------------------------------------
 ...
 */
export function resolveClaudeCodeInheritance(): ClaudeCodeInheritanceConfig {
 ...
 */
export function loadClaudeCodeSettings(): Effect.Effect<
 ...
 */
export function findClaudePromptsDir(): Effect.Effect<string | null> {
  return Effect.gen(function* () {
    const inherit = resolveClaudeCodeInheritance()
...
export const PROVIDER_FACTORIES: Record<string, ProviderFactory> = {
   ...
   * Uses MOMO_CUSTOM_BASE_URL, MOMO_CUSTOM_API_KEY, MOMO_CUSTOM_MODEL env vars.
   */
  custom: () => ({
...
export function listAvailableProviders(): Array<{
  ...
  return [
    ...
    { name: "zhipu", envSuffix: "ZHIPU", description: "Zhipu GLM models (智谱 AI)" },
    { name: "minimax", envSuffix: "MINIMAX", description: "MiniMax models" },
    { name: "moonshot", envSuffix: "MOONSHOT", description: "Moonshot Kimi models (月之暗面)" },
    { name: "stepfun", envSuffix: "STEPFUN", description: "StepFun models (阶跃星辰)" },
    { name: "doubao", envSuffix: "DOUBAO", description: "Doubao models (字节跳动)" },
    { name: "custom", envSuffix: "CUSTOM", description: "Custom OpenAI-compatible provider" },
...
export function resolveTierModel(
  tierName: string,
): Effect.Effect<string | null, never, Auth> {
  return Effect.gen(function* () {
    const tierModels =
      BUILTIN_TIERS[tierName as keyof typeof BUILTIN_TIERS]
...
export function resolveModel(
  identifier: string,
): Effect.Effect<ModelGroupEntry, ModelResolutionError, Auth> {
  return Effect.gen(function* () {
    ...
    if (identifier.includes(":")) {
      ...
      const entry = Array.from(modelMap.values()).find(
        (m) =>
...
export function expandModelId(
  identifier: string,
): Effect.Effect<string, ModelResolutionError, Auth> {
  return Effect.gen(function* () {
 ...
 */
export class Provider extends Effect.Service<Provider>()("Provider", {
  effect: Effect.gen(function* () {
    ...
    const providerConfigCache = yield* Ref.make<
      Map<string, ProviderFactoryResult>
    >(new Map())

    /** Cache of resolved credentials per provider. */
    const credentialCache = yield* Ref.make<
      Map<string, ProviderCredentials | null>
    >(new Map())
    ...
    const getProviderConfig = (
      name: string,
    ): Effect.Effect<ProviderFactoryResult, ProviderInitializationError> =>
      Effect.gen(function* () {
    ...
    const getCredentials = (
      providerName: string,
    ): Effect.Effect<ProviderCredentials | null> =>
      Effect.gen(function* () {
     ...
     */
    const resolveChunkTimeout = (
      providerName: string,
    ): Effect.Effect<number, ProviderInitializationError> =>
      Effect.gen(function* () {
    ...
    const loadMergedConfig = Effect.gen(function* () {
      ...
      const merged: typeof userConfig = {
        ...claudeSettings,
        ...userConfig,
        // Deep-merge provider configs if both have them
        providers: {
          ...(claudeSettings.providers as typeof userConfig.providers),
    ...
    const createModel = (
      providerName: string,
      modelId: string,
    ): Effect.Effect<UnifiedLanguageModel, ProviderError | ProviderInitializationError> =>
      Effect.gen(function* () {
        const providerConfig = yield* getProviderConfig(providerName)
     ...
     */
    const resolveAndCreate = (
      identifier?: string,
    ): Effect.Effect<UnifiedLanguageModel, ProviderError | ProviderInitializationError, Auth> =>
      Effect.gen(function* () {
        // Use identifier from args, env, config, or default tier
        const resolvedIdentifier =
          identifier ||
          (yield* env.getString("MOMO_MODEL")) ||
          (yield* config.getOptional<string>("model")) ||
          "standard"

        // Check if it's a tier
        const tierName =
          BUILTIN_TIERS[resolvedIdentifier as keyof typeof BUILTIN_TIERS]
            ? resolvedIdentifier
            : null
        ...
        const resolved = yield* Effect.either(
          resolveModel(resolvedIdentifier),
        )
        ...
        const defaultProvider =
          (yield* env.getString("MOMO_PROVIDER")) ||
          (yield* config.getOptional<string>("provider")) ||
          "llmgateway"
    ...
    const getCurrentModel = Effect.gen(function* () {
      ...
      return {
        ...
        entry:
          modelMap.get(state.resolvedModelId!) ||
          ({
    ...
    const switchModel = (
      ...
      Effect.gen(function* () {
        ...
        yield* instanceState.setModel(
          newModel.model,
          newModel.modelId,
          newModel.provider,
          identifier in BUILTIN_TIERS,
        )
        return newModel
      })
     ...
     */
    const getClaudeCodeInheritance = Effect.sync(() =>
    ...
    const listAllModels = Effect.gen(function* () {
      const models = Array.from(
        new Map(BUILTIN_MODELS.map((m) => [m.id, m])).values(),
      )

      // Check credentials for each
      const withAvailability = yield* Effect.forEach(models, (model) =>
        Effect.gen(function* () {
    ...
    const getModelInfo = (
      modelId: string,
    ): Effect.Effect<ModelGroupEntry | null> =>
      Effect.sync(() => modelMap.get(modelId) || null)
