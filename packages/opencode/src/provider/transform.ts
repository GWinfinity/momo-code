        ...
        switch (targetProvider) {
          case "anthropic": {
            ...
            if (
              ...
              const systemMsg = transformed.messages.find(
          ...
          case "cohere": {
            ...
            if (Array.isArray(transformed.messages)) {
              const userMsgs = transformed.messages.filter(
                (m: Record<string, unknown>) => m.role === "user",
              )
              const assistantMsgs = transformed.messages.filter(
              ...
              if (assistantMsgs.length > 0) {
                transformed.chat_history = assistantMsgs.map(
        ...
        switch (sourceProvider) {
          ...
          case "google-vertex": {
            ...
            if (
              ...
              const candidate = (chunk.candidates as Array<Record<string, unknown>>)[0]
              if (
                candidate.content &&
