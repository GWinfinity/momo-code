...
import { formatLedgerEntry, parseLedgerEntry } from "./ledger"
 ...
 */
function ensureDir(): Effect.Effect<void, Error> {
	return Effect.try({
		try: () => {
			...
			new Error(
				`Failed to create experience directory: ${error instanceof Error ? error.message : String(error)}`,
			),
	})
}
 ...
 * ```
 */
export class ExperienceStore extends Effect.Service<ExperienceStore>()(
	"experience/Store",
	{
		effect: Effect.gen(function* () {
			// In-memory caches for fast access
			const tacticsRef = yield* Ref.make<ReadonlyArray<Tactic>>([])
			const casesRef = yield* Ref.make<ReadonlyArray<Case>>([])
			const ledgerLoadedRef = yield* Ref.make<boolean>(false)
			 ...
			 */
			const loadTactics = (): Effect.Effect<ReadonlyArray<Tactic>, Error> =>
				Effect.gen(function* () {
					type TacticFile = { tactics: Tactic[] }
					const data = yield* readJsonFile<TacticFile>(
						TACTICS_FILE,
						{ tactics: [] },
					)
			 ...
			 */
			const getCachedTactics = (): Effect.Effect<ReadonlyArray<Tactic>> =>
				Ref.get(tacticsRef)
				...
				Effect.gen(function* () {
					...
					if (index === -1) {
						yield* Effect.logWarning(
							`[ExperienceStore] Tactic not found for status update: ${id}`,
						)
						return false
					}

					const updated = tactics.map((t, i) =>
						i === index ? { ...t, status } : t,
					)

					yield* saveTactics(updated)
					yield* Effect.log(
						`[ExperienceStore] Updated tactic ${id} status → ${status}`,
					)
					return true
				})
			 ...
			 */
			const loadCases = (): Effect.Effect<ReadonlyArray<Case>, Error> =>
				Effect.gen(function* () {
					type CasesFile = { cases: Case[] }
					const data = yield* readJsonFile<CasesFile>(CASES_FILE, {
						cases: [],
					})
			 ...
			 */
			const appendCase = (newCase: Case): Effect.Effect<void, Error> =>
				Effect.gen(function* () {
			 ...
			 */
			const getCachedCases = (): Effect.Effect<ReadonlyArray<Case>> =>
				Ref.get(casesRef)
				...
				Effect.gen(function* () {
					...
					yield* Effect.try({
						try: () => {
							...
							new Error(
								`Failed to append ledger entry: ${error instanceof Error ? error.message : String(error)}`,
							),
					})
				...
				Effect.gen(function* () {
					...
					yield* Effect.try({
						try: () => {
							...
							new Error(
								`Failed to append ledger batch: ${error instanceof Error ? error.message : String(error)}`,
							),
					})

					yield* Effect.log(
						`[ExperienceStore] Appended ${entries.length} ledger entries`,
					)
				})
			 ...
			 */
			const loadLedger = (): Effect.Effect<
			...
			> =>
				Effect.gen(function* () {
					...
					if (skipped > 0) {
						yield* Effect.logWarning(
							`[ExperienceStore] Skipped ${skipped} invalid ledger lines`,
						)
					}

					yield* Ref.set(ledgerLoadedRef, true)
					yield* Effect.log(
						`[ExperienceStore] Loaded ${entries.length} ledger entries`,
					)
					return entries
				})
			...
			> =>
				Effect.gen(function* () {
					...
					const [tactics, cases, ledger] = yield* Effect.all(
						[loadTactics(), loadCases(), loadLedger()],
						{ concurrency: 3 },
					)

					yield* Effect.log(
						`[ExperienceStore] Ready: ${tactics.length} tactics, ${cases.length} cases, ${ledger.length} ledger entries`,
					)
