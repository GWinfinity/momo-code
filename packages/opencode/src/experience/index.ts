...
import type { DistillResult } from "./distiller"
...
export interface EvolveOpts {
  ...
  readonly review?: boolean        // Human review mode
...
export const Evolve = (opts: EvolveOpts) =>
  Effect.gen(function* () {
    ...
    const distilled = yield* distiller.distill(
      opts.signals,
      { dedup: true },
      existingTactics,
    )
...
export const InjectForTask = (task: TaskCtx) =>
  Effect.gen(function* () {
