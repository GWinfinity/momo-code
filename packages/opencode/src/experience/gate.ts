...
import { winRate, canActivate, canPromote, shouldRetire } from "./tactic"
import { ExperienceStore } from "./store"
import type { PromoteEntry, RetireEntry } from "./ledger"
import { XP_CONFIG, getModeTuning } from "./config"
 ...
 */
export class Gate extends Effect.Service<Gate>()("experience/Gate", {
  effect: Effect.gen(function* () {
     ...
     */
    const maybePromote = (tactic: Tactic, opts?: GateOpts) =>
      Effect.gen(function* () {
        ...
        yield* Effect.log(
          `[Gate] Evaluating promotion for ${tactic.id} ` +
        ...
        if (tactic.status === "draft") {
          ...
          yield* Effect.log(
            `[Gate] draft→active check: winRate=${wr.toFixed(3)} ` +
          ...
          if (hasEnoughUses && meetsThreshold) {
            ...
            const promoteEntry: PromoteEntry = {
              kind: "promote",
        ...
        if (tactic.status === "active") {
          ...
          yield* Effect.log(
            `[Gate] A/B result: with=${abResult.withTactic.toFixed(3)} ` +
          ...
          yield* Effect.log(
            `[Gate] Ratchet: ${ratchet.passed ? "PASS ✅" : "FAIL ❌"} ` +
          ...
          if (ratchet.passed) {
            ...
            yield* Ref.update(promotionHistoryRef, (h) => [
              ...h,
              {
                timestamp: Date.now(),
                tacticId: tactic.id,
                fromStatus: "active" as TacticStatus,
                toStatus: "promoted" as TacticStatus,
            ...
            return "promoted" as TacticStatus
          }
     ...
     */
    const abTestWithWithout = (tactic: Tactic, bench: string) =>
      Effect.gen(function* () {
        ...
        yield* Effect.log(
          `[Gate] A/B complete: with=${withScore.toFixed(3)} without=${withoutScore.toFixed(3)} ` +
     ...
     */
    const ratchetCheck = (scores: AbTestResult) =>
      Effect.gen(function* () {
        ...
        yield* Effect.log(
          `[Gate] Ratchet: ${regressionCount} regressions ` +
     ...
     */
    const maybeRetire = (tactic: Tactic) =>
      Effect.gen(function* () {
        ...
        yield* Effect.log(
          `[Gate] Retiring tactic ${tactic.id}: ` +
        ...
        yield* Ref.update(promotionHistoryRef, (h) => [
          ...h,
          {
            timestamp: Date.now(),
            tacticId: tactic.id,
            fromStatus: tactic.status as TacticStatus,
            toStatus: "retired" as TacticStatus,
